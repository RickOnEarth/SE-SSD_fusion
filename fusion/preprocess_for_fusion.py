...
def preprocess_for_fusion(model, example, all_preds_dicts, train_flag=True):
    ...
    batch_cls_preds = preds_dict["cls_preds"].view(batch_size, -1, num_class_with_bg)  # [bs=1, 70400/107136, 1]
    batch_box_preds = preds_dict["box_preds"].view(batch_size, -1, model.bbox_head.box_n_dim)  # [bs=1, 70400/107136, 7]
    batch_task_anchors = example["anchors"][task_id].view(batch_size, -1, model.bbox_head.box_n_dim)   # [bs=1, 70400/107136, 7]

    batch_anchors_mask = [None] * batch_size
    if "anchors_mask" in example and train_flag is False:       #for pointpillars inference
        batch_anchors_mask = example['anchors_mask'][task_id].view(batch_size, -1)                   #code not tested
    ...
    for box_preds, cls_preds, rect, Trv2c, P2, metadata, a_mask in zip(
        ...
        anchors = example['anchors'][task_id].view(-1, model.bbox_head.box_coder.n_dim)
        ...
        final_box_preds = model.bbox_head.box_coder.decode_torch(box_preds, anchors)        #decode!
