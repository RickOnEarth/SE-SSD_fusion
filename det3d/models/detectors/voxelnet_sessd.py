    ...
    def forward(self, example, is_ema=[False, None], return_loss=True, **kwargs):
        ...
        if is_ema[0]:
            ...
            if return_loss:
                ...
                ret = self.bbox_head.predict(example, preds, self.test_cfg)
                return ret


@DETECTORS.register_module
class VoxelNet_FUSION_MX(SingleStageDetector):

    def __init__(self, reader, backbone, neck, bbox_head, train_cfg=None, test_cfg=None, pretrained=None,):
        super(VoxelNet_FUSION_MX, self).__init__(reader, backbone, neck, bbox_head, train_cfg, test_cfg, pretrained)
