...
def rotation_points_single_angle(points, angle, axis=0):
    ...
    if axis == 1:
        rot_mat_T = torch.stack([
            ...
            torch.tensor([0., 1., 0.]),
            torch.tensor([rot_sin, 0., rot_cos])
        ])
    elif axis == 2 or axis == -1:
        rot_mat_T = torch.stack([
            torch.tensor([rot_cos, -rot_sin, 0.]),
            torch.tensor([rot_sin, rot_cos, 0.]),
            torch.tensor([0., 0., 1.])
        ])
    elif axis == 0:
        rot_mat_T = torch.stack([
            torch.tensor([1., 0., 0.]),
            torch.tensor([0., rot_cos, -rot_sin]),
            torch.tensor([0., rot_sin, rot_cos])
        ])
    else:
        raise ValueError("axis should in range")
    return points @ rot_mat_T.to(points.device)
...
def rotate_weighted_nms(
    ...
    if enable_centerness and not centerness_c:
        dist_boxes_anchors = torch.abs(box_preds - anchors)
        dist_metric = torch.softmax(torch.pow(torch.pow(dist_boxes_anchors[:, 0:2], 2).sum(-1), 0.5), dim=0)
        dist_metric = torch.ones_like(dist_metric) - dist_metric
        scores *= torch.pow(dist_metric, centerness_pow)
    ...
    if enable_centerness and centerness_c:
        anchors_np = anchors.cpu().numpy()
    else:
        anchors_np = None

    if len(dets_np) == 0:
        box_ret_np,  dir_ret_list, labels_ret_list, scores_ret_list, selected = [np.array([], dtype=np.int64)] * 5
    else:
        nms_result = rotate_weighted_nms_cc(box_preds_np,
                                            ...
                                            dir_labels_np,
                                            anchors_np,
                                            nms_cnt_thresh=nms_cnt_thresh,
                                            nms_sigma_dist_interval=nms_sigma_dist_interval,
                                            nms_sigma_square=nms_sigma_square,
                                            suppressed_thresh=suppressed_thresh,
                                            )
